...
from milvus_vectordb import milvus_connect, milvus_disconnect, create_text_collection, search_similar_images, format_genre
...
def main():
    try:
        ...
        results = search_similar_images(collection, model, "space adventure", top_k=5)
        ...
        results = search_similar_images(collection, model, "space adventure", top_k=5, min_rating=7.0)
        for i, m in enumerate(results, 1):
            logger.info(f"  {i}. {m['title']} ({m['release_date'][:4]}) - Rating: {m['vote_average']} - {m['similarity_percent']}")
        
        logger.info("Query 3: Recent action movies (2020+, genre='Action')")
        results = search_similar_images(collection, model, "intense action thriller", top_k=5, genre_filter="Action", min_year=2020)
        for i, m in enumerate(results, 1):
        ...
        results = search_similar_images(collection, model, "romantic comedy", top_k=5, min_rating=6.0, max_rating=8.0, min_popularity=50, genre_filter="Romance")
        for i, m in enumerate(results, 1):
        ...
        results = search_similar_images(collection, model, "science fiction thriller", top_k=5, min_year=1990, max_year=1999, min_rating=7.0, genre_filter="Science Fiction")
        for i, m in enumerate(results, 1):
            logger.info(f"  {i}. {m['title']} ({m['release_date'][:4]}) - Rating: {m['vote_average']} - {m['similarity_percent']}")
        
        logger.info("Query 6: High-rated animated (min_rating=7.5, genre='Animation')")
        results = search_similar_images(collection, model, "family adventure", top_k=5, min_rating=7.5, genre_filter="Animation")
        for i, m in enumerate(results, 1):
            logger.info(f"  {i}. {m['title']} ({m['release_date'][:4]}) - Rating: {m['vote_average']} - {m['similarity_percent']}")
        
        logger.info("Query 7: Movies from 2021 (year_filter=2021)")
        results = search_similar_images(collection, model, "action adventure", top_k=5, year_filter=2021)
        for i, m in enumerate(results, 1):
        ...
        results = search_similar_images(collection, model, "drama", top_k=5, max_year=1980, min_rating=7.0)
        for i, m in enumerate(results, 1):
            logger.info(f"  {i}. {m['title']} ({m['release_date'][:4]}) - Rating: {m['vote_average']} - {m['similarity_percent']}")
